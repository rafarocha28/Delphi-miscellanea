...
uses
  ...
  SDQC.Forecourt.Factory, SDQC.Config;
...
type
  TMainForm = class(TForm)
    ...
    procedure ConfigSerialButtonClick(Sender: TObject);
  private
    FRunMode: TRunMode;
    ...
    FSerialConfig: TSerialConfiguration;
...
uses
  ...
  Util.Kkversion,
  Util.HH, SDQC.View.SerialConfig;
...
procedure TMainForm.ConfigSerialButtonClick(Sender: TObject);
var
  scf: TSerialConfigForm;
begin
  scf := TSerialConfigForm.Create(Application);
  try
    scf.Font := Self.Font;
    scf.ShowModal;
  finally
...
begin
  ...
  WriteEthernet(AnsiString(ReplyEthernetMemo.Text), ASocket);
end;
